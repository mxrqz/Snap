...
export function queryToRequest(query: ValidatedSnapQuery): ValidatedSnapRequest {
  ...
  if (query['viewport.width'] || query['viewport.height'] || query['viewport.isMobile'] || query['viewport.deviceScaleFactor']) {
    ...
    if (query['viewport.isMobile']) request.screenshot!.viewport.isMobile = query['viewport.isMobile'];
